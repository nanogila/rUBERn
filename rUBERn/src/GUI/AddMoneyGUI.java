...
private Matrix theMatrix;
	 ...
	 * Create the frame.
	 */
	public AddMoneyGUI(Matrix aMatrix, Person aPerson) {
		setResizable(false);
		thePerson = aPerson;
	...
	private class SwingAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (rawMoney.length() < 11 && rawMoney.length() > 0 && rawMoney.matches("[0-9]+")) {
				...
				if (money > 0) {
					thePerson.addMoney(money);
					User theUser;
					Driver theDriver;
					if (thePerson instanceof User) {
						theUser = (User)thePerson;
						new PostLogin(theMatrix, theUser);
						new Error ("Added $"+money+" successfully");
						dispose();
					}else if (thePerson instanceof Driver) {
						theDriver = (Driver)thePerson;
